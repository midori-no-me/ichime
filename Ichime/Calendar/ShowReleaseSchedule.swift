...
struct ShowReleaseSchedule {
  ...
  func getSchedule() async throws -> [GroupedShowsFromCalendar] {
    ...
    let showsFromCalendar = calendarEntries.map { ShowFromCalendar.createFromShikimoriApi(
    ...
    for showFromCalendar in showsFromCalendar {
      showsGroupedByDate[Calendar.current.startOfDay(for: showFromCalendar.nextEpisodeReleaseDate), default: []]
    ...
    for (date, showsFromCalendar) in showsGroupedByDate {
      items.append(.init(date: date, shows: showsFromCalendar.sorted(by: {$0.nextEpisodeReleaseDate < $1.nextEpisodeReleaseDate})))
    }
