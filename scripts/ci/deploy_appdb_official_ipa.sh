...
post_form() {
  ...
  local http_code
  http_code="$(curl -sS -w "%{http_code}" -o "$response_file" -X POST "$API_BASE_URL/$endpoint/" "$@")"
  [[ "$http_code" =~ ^2 ]] || {
    sed -n '1,200p' "$response_file" >&2
    fail "appdb /$endpoint/ returned HTTP $http_code"
  }
...
poll_official_ipa() {
  ...
  for ((attempt = 1; attempt <= POLL_ATTEMPTS; attempt++)); do
    post_form "get_official_ipas" "$response_file" \
      --data-urlencode "st=$APPDB_ST" \
      --data-urlencode "brand=appdb" \
      --data-urlencode "lang=en" \
      --data-urlencode "scope=$APPDB_APP_IDENTIFIER" \
      --data-urlencode "sha1_hash=$sha1_hash"
...
IPA_ID="$(poll_official_ipa "$IPA_SHA1" "$RESPONSE_FILE")"
