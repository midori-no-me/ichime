...
class ContentProvider: TVTopShelfContentProvider {
    let cookieStorage = HTTPCookieStorage.sharedCookieStorage(forGroupContainerIdentifier: ServiceLocator.appGroup)
    ...
    var api: ScraperAPI.APIClient {
        .init(baseURL: ServiceLocator.websiteBaseUrl, userAgent: ServiceLocator.userAgent, session: session)
    }
