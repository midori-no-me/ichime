...
extension ScraperAPI.Types {
  public struct Moment: Identifiable, Hashable {
    ...
    init(from element: Element, withUser user: User?, baseURL: URL) throws {
      guard
        let strId = try? element.select(".m-moment__title a").attr("href").split(separator: "/")
          .last,
        let id = Int(strId),
        let title = try? element.select(".m-moment__title a").text(
        ...
        let preview = try? element.select(".m-moment__thumb.a img").attr("src").split(
          separator: "?"
        ).first,
        let fromAnime = try? element.select(".m-moment__episode").text(
          trimAndNormaliseWhitespace: true
        ),
    ...
    init(from element: Element, baseURL: URL) throws {
      if let userName = try? element.select(".m-moment-author-name").text(
        trimAndNormaliseWhitespace: true
      ),
        let userIdStr = try? element.select(".m-moment-author-name a").attr("href").split(
          separator: "/"
        )
        .item(at: 1),
