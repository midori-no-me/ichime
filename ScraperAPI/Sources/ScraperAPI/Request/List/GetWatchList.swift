...
extension ScraperAPI.Request {
  public struct GetWatchList: ScraperHTMLRequest {
    ...
    public func parseResponse(html: String, baseURL: URL) throws -> [ScraperAPI.Types
      .ListByCategory]
    {
      do {
        ...
        if let type, sections.size() == 1 {
          return try sections.array().map {
            try ScraperAPI.Types.ListByCategory(from: $0, type: type)
          }
        }
