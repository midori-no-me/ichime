...
public extension ScraperAPI.Request {
    struct GetNextToWatch: ScraperHTMLRequest {
        ...
        public func parseResponse(html: String, baseURL: URL) throws -> [ScraperAPI.Types.WatchShow] {
            do {
                ...
                return elements.array().compactMap { try? ScraperAPI.Types.WatchShow(from: $0) }
            } catch {
                logger.error("\(String(describing: Self.self)): cannot parse, \(error.localizedDescription, privacy: .public)")
                throw ScraperAPI.APIClientError.parseError
            }
        }
    }
