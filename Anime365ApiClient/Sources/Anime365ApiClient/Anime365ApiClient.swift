        ...
        while attempt < maxRetries {
            ...
            do {
                ...
                if let httpResponse = httpResponse as? HTTPURLResponse {
                    logger.notice("[Anime365ApiClient] API response: GET \(httpRequest.url?.absoluteString ?? "") [\(httpResponse.statusCode)]")
                }
                ...
                lastError = error
                logger.error("[Anime365ApiClient] Request failed on attempt \(attempt): \(error.localizedDescription, privacy: .public)")
